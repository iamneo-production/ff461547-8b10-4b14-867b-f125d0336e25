...
const CarBookingForm = () => {
  ...
  const [dropoffDate, setDropoffDate] = useState("");
   ...
   axios.post("http://localhost:8080/bookform",json).then(response=>console.log(response)).catch(err=>console.log(err)); }
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/findcarbyId?carid=${id}`)
      .then((response) => {
              ...
              <div className="text-center">
                <div className="d-flex justify-content-center align-items-center ">
                  <button className="booking__form car__item-btn car__btn-rent" onClick={sub} style={{ flex: "0 0 auto", borderRadius: "5px", minWidth: "200px" ,margin:"10px"}}>
                    <Link to="/car-rentals" className="text-black no-underline">Proceed to Pay
                    </Link>
                  </button>
                </div>
              </div>
