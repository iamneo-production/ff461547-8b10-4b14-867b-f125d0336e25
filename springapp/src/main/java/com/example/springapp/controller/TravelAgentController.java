...
public class TravelAgentController {
    ...
    @GetMapping("/travelAgents")
    public ResponseEntity<?> getAllTravelAgents() {
        List<TravelAgent> travelAgents=travelAgentService.getAllTravelAgents();
        return travelAgents==null? ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ErrorJsonDto("Something went wrong, Try again", 500))
        : ResponseEntity.status(HttpStatus.OK).body(travelAgents);
    }
